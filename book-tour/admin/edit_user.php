...
if (isset($_POST['sbm'])) {
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $sql = "UPDATE user
				SET user_name = '$user_name', user_mail = '$user_mail', user_pass = '$user_pass', user_level = $user_level
				WHERE user_id = $user_id";
        $query = mysqli_query($conn, $sql);
                        ...
                        <form role="form" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <select name="user_level" class="form-control">
                                    <option value=1 <?php if ($row['user_level'] == 1) {
                                                        echo 'selected';
                                                    } ?>>Admin</option>
                                    <option value=2 <?php if ($row['user_level'] == 2) {
                                                        echo 'selected';
                                                    } ?>>Member</option>
                                </select>
                            </div>
